...
if (isset($_FILES['proformaFichier']) && $_FILES['proformaFichier']['error'] === UPLOAD_ERR_OK) {
    ...
    $file = $_FILES['proformaFichier'];
    $fileName = $file['name'];
    $fileTmpPath = $file['tmp_name'];
    $fileSize = $file['size'];
    $fileExtension = strtolower(pathinfo($fileName, PATHINFO_EXTENSION));

    // 1. Vérifier la taille (max 10 Mo)
    $maxFileSize = 10 * 1024 * 1024; // 10 Mo
    if ($fileSize > $maxFileSize) {
        $_SESSION['error'] = "Le fichier est trop volumineux (max 10 Mo).";
    }
    // 2. Vérifier l'extension
    elseif (!in_array($fileExtension, ['pdf', 'doc', 'docx', 'jpg', 'png', 'jpeg'])) {
    ...
    // 3. Vérifier le type MIME réel
        ...
        if (!in_array($mimeType, $allowedMimeTypes)) {
            $_SESSION['error'] = "Type de fichier non autorisé ($mimeType).";
        } else {
            // --- Récupération du dossier_upload du besoin ---
            $stmtDossier = $pdo->prepare("SELECT dossier_upload FROM besoins WHERE id = ?");
            $stmtDossier->execute([$besoin_id]);
            $dossier_upload = $stmtDossier->fetchColumn();

            if (empty($dossier_upload)) {
                // Si le besoin n'a pas de dossier, on le génère à partir de son titre
                $stmtTitre = $pdo->prepare("SELECT titre FROM besoins WHERE id = ?");
                $stmtTitre->execute([$besoin_id]);
                $titre = $stmtTitre->fetchColumn();
                // Utiliser la fonction genererNomDossierBesoin (doit être définie ailleurs)
                if (function_exists('genererNomDossierBesoin')) {
                    $dossier_upload = genererNomDossierBesoin($pdo, $titre, $besoin_id);
                } else {
                    // Fallback simple si la fonction n'existe pas
                    $dossier_upload = slugify($titre);
                    if (empty($dossier_upload)) $dossier_upload = 'document';
                }
                // Mettre à jour le besoin
                $update = $pdo->prepare("UPDATE besoins SET dossier_upload = ? WHERE id = ?");
                $update->execute([$dossier_upload, $besoin_id]);
            }

            // --- Créer le sous-dossier 'proforma' dans le dossier du besoin ---
            $proformaDir = __DIR__ . '/uploads/' . $dossier_upload . '/proforma/';
            if (!is_dir($proformaDir)) {
                mkdir($proformaDir, 0755, true);
            }

            // 4. Générer un nom de fichier unique
            $newFileName = 'TDR_' . time() . '_' . rand(100, 999) . '.' . $fileExtension;

            // 5. Définir le chemin de destination
            $destPath = $proformaDir . $newFileName;

            // 6. Déplacer le fichier
            if (move_uploaded_file($fileTmpPath, $destPath)) {
                $fichier_nom = $newFileName;
            } else {
                $_SESSION['error'] = "Erreur lors de l'enregistrement du fichier sur le serveur.";
            }
        }
    }
}
// Si le fichier est optionnel, on ne fait rien de plus.
// Si vous souhaitez le rendre obligatoire, ajoutez une vérification après ce bloc.
        try {
            ...
            try {
                ...
                $mail->isSMTP();
                $mail->Host       = 'smtp.gmail.com'; //  smtp.gmail.com si c'est  Gmail; smtp.office365.com pour Outlook
                $mail->SMTPAuth   = true;
                $mail->Username   = 'koubouraadidjatouidrissouissa@gmail.com'; 
                $mail->Password   = '1309papaokk';              
                $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
                $mail->Port       = 587;
                $mail->CharSet    = 'UTF-8';

                $mail->setFrom('koubouraadidjatouidrissouissa@gmail.com', 'Logistique Swisscontact');
