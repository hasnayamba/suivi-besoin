...
// --- Fonction pour déterminer si un contrat est en alerte (expiration dans 60j ou déjà expiré) ---
function isContratInAlerte($row) {
    if ($row['statut'] === 'Expiré') return true;
    if ($row['statut'] === 'En cours' && !empty($row['date_fin_prevue'])) {
        $dateFin = new DateTime($row['date_fin_prevue']);
        $limiteAlerte = new DateTime('+60 days');
        return $dateFin <= $limiteAlerte;
    }
    return false;
}
                            ...
                            <tr class="<?= $estExpire ? 'bg-expired' : '' ?>">
                                ...
                                <td class="text-end pe-3">
                                    <div class="btn-group action-btn-group">
                                        <a href="detail_contrat.php?id=<?= $c['id'] ?>" class="btn btn-sm btn-outline-info" title="Consulter"><i class="bi bi-eye"></i></a>
                                        
                                        <!-- Le bouton Modifier est totalement supprimé dans la zone d'alerte -->
                    ...
                    <table id="tableContrats" class="table table-hover" style="width:100%">
                        ...
                        <tbody>
                            <?php foreach ($contrats as $row): 
                                $inAlerte = isContratInAlerte($row);
                            ?>
                                <tr>
                                    ...
                                    <td class="text-end no-print">
                                        <div class="btn-group action-btn-group">
                                            <a href="detail_contrat.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-outline-info" title="Voir fiche"><i class="bi bi-eye"></i></a>
                                            
                                            <!-- Modifier : apparaît seulement si contrat en cours ET NON en alerte -->
                                            <?php if($row['statut'] === 'En cours' && !$inAlerte): ?>
