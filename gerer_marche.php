...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$is_locked) {
    ...
    // --- Récupération du dossier_upload du besoin associé au marché ---
    $stmt_dossier = $pdo->prepare("SELECT b.dossier_upload 
                                   FROM marches m 
                                   JOIN besoins b ON m.besoin_id = b.id 
                                   WHERE m.id = ?");
    $stmt_dossier->execute([$marche_id]);
    $dossier_upload = $stmt_dossier->fetchColumn();

    // Si le besoin n'a pas encore de dossier (ancien besoin), on le génère à partir de son titre
    if (empty($dossier_upload)) {
        $stmt_titre = $pdo->prepare("SELECT b.titre 
                                     FROM marches m 
                                     JOIN besoins b ON m.besoin_id = b.id 
                                     WHERE m.id = ?");
        $stmt_titre->execute([$marche_id]);
        $titre = $stmt_titre->fetchColumn();
        // Utiliser la fonction genererNomDossierBesoin (à définir si non déjà présente)
        $dossier_upload = genererNomDossierBesoin($pdo, $titre, null); // null car on ne connaît pas l'id du besoin ici, on peut le passer plus tard
        // Mettre à jour le besoin avec ce dossier
        $stmt_update = $pdo->prepare("UPDATE besoins SET dossier_upload = ? WHERE id = (SELECT besoin_id FROM marches WHERE id = ?)");
        $stmt_update->execute([$dossier_upload, $marche_id]);
    }

    // Créer le dossier physique si nécessaire
    $uploadDir = __DIR__ . '/uploads/' . $dossier_upload . '/';
    if (!is_dir($uploadDir)) {
    ...
    if (isset($_POST['submit_document'])) {
        ...
        if (!empty($document_type) && isset($_FILES['fichier_document']) && $_FILES['fichier_document']['error'] === UPLOAD_ERR_OK) {
            ...
            // Utilisation du dossier spécifique au besoin
    ...
    elseif (isset($_POST['update_document'])) {
        ...
        if (isset($_FILES['nouveau_fichier']) && $_FILES['nouveau_fichier']['error'] === UPLOAD_ERR_OK) {
            // Utilisation du dossier spécifique au besoin (le même que précédemment)
            // Supprimer l'ancien fichier s'il existe dans le dossier
            if (!empty($fichier_actuel) && file_exists($uploadDir . $fichier_actuel)) {
                unlink($uploadDir . $fichier_actuel);
            }
