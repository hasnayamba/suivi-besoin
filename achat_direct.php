...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_achat_direct'])) {
    ...
    $stmt_dossier->execute([$besoin_id]);
    $dossier_upload = $stmt_dossier->fetchColumn();

    // Si le besoin n'a pas encore de dossier (ancien besoin), on le génère
    if (empty($dossier_upload)) {
        // Récupérer le titre du besoin
        $stmt_titre = $pdo->prepare("SELECT titre FROM besoins WHERE id = ?");
        $stmt_titre->execute([$besoin_id]);
        $titre = $stmt_titre->fetchColumn();
        // Générer un nom de dossier unique (fonction slugify et genererNomDossierBesoin à définir ailleurs ou ici)
    ...
    // --- Fonction d'upload modifiée pour utiliser le dossier du besoin ---
    function handle_upload($file_key, $marche_id, $doc_type, $pdo, $uploadDir) {
    ...
    try {
        ...
        handle_upload('facture', $marche_id, 'Facture', $pdo, $uploadDir);
        handle_upload('proforma', $marche_id, 'Proforma', $pdo, $uploadDir);
        handle_upload('bon_commande', $marche_id, 'Bon de Commande', $pdo, $uploadDir);
        handle_upload('bon_livraison', $marche_id, 'Bon de Livraison', $pdo, $uploadDir);
