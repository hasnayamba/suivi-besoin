                ...
                <div class="card border-0 shadow-sm h-100">
                    <div class="card-body p-4">
                        ...
                        <div class="mt-4 pt-3 border-top">
                            <h6 class="fw-bold mb-3"><i class="bi bi-file-earmark-text text-primary me-2"></i>Pièce jointe (TDR / CDC)</h6>
                            <?php if(!empty($besoin['fichier'])): ?>
                                <button type="button" class="btn btn-outline-primary shadow-sm" data-bs-toggle="modal" data-bs-target="#tdrModal">
                                    <i class="bi bi-eye-fill me-2"></i>Visionner le TDR / Cahier des Charges
                                </button>
                            <?php else: ?>
                                <div class="alert alert-secondary border-0 py-2 small mb-0 d-inline-block">
                                    <i class="bi bi-info-circle me-1"></i> Aucun fichier n'a été rattaché à cette demande.
                                </div>
                            <?php endif; ?>
                        </div>
...
<?php if(!empty($besoin['fichier']) && !empty($besoin['dossier_upload'])): ?>
<div class="modal fade" id="tdrModal" tabindex="-1">
    <div class="modal-dialog modal-xl modal-dialog-centered">
        <div class="modal-content shadow-lg">
            <div class="modal-header bg-light">
                <h5 class="modal-title fw-bold text-primary"><i class="bi bi-display me-2"></i>Visionneuse : TDR / Cahier des Charges</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body p-0 bg-dark" style="height: 80vh;">
                <?php 
                $ext = strtolower(pathinfo($besoin['fichier'], PATHINFO_EXTENSION));
                
                // Chemin relatif pour l'URL (pour l'iframe)
                $fichierPath = "uploads/" . rawurlencode($besoin['dossier_upload']) . "/" . rawurlencode($besoin['fichier']);
                
                // Chemin absolu pour vérifier l'existence du fichier sur le serveur
                // Le fichier se trouve dans le même répertoire que ce script, donc on part de __DIR__
                $fullServerPath = __DIR__ . '/' . $fichierPath;
                // Si le script est dans un sous-dossier, ajustez le chemin (par exemple __DIR__ . '/../' . $fichierPath)
                // Ici on suppose que le script est dans le dossier racine du projet
                
                $fileExists = file_exists($fullServerPath);
                
                if (!$fileExists): ?>
                    <div class="d-flex flex-column align-items-center justify-content-center h-100 text-white p-5 text-center">
                        <i class="bi bi-file-earmark-slash fs-1 mb-3"></i>
                        <p>Le fichier est introuvable sur le serveur.<br>
                        <small>Chemin recherché : <?= htmlspecialchars($fichierPath) ?></small></p>
                    </div>
                <?php else: 
                    // Construction de l'URL complète pour l'iframe (nécessaire pour Word Online)
                    $baseUrl = "http://" . $_SERVER['HTTP_HOST'] . rtrim(dirname($_SERVER['PHP_SELF']), '/\\');
                    $fullUrl = $baseUrl . "/" . $fichierPath;

                    if ($ext === 'pdf'): ?>
                        <iframe src="<?= $fichierPath ?>#toolbar=0&navpanes=0" width="100%" height="100%" style="border: none;"></iframe>
                    <?php elseif (in_array($ext, ['doc', 'docx'])): ?>
                        <?php if ($_SERVER['HTTP_HOST'] === 'localhost' || $_SERVER['HTTP_HOST'] === '127.0.0.1'): ?>
                            <div class="d-flex flex-column align-items-center justify-content-center h-100 text-white p-5 text-center">
                                <i class="bi bi-exclamation-triangle text-warning fs-1 mb-3"></i>
                                <h4>Visionneuse Word indisponible en local</h4>
                                <p>La visionneuse Microsoft a besoin d'une adresse internet publique pour fonctionner.<br>Cela fonctionnera parfaitement une fois le site en ligne.</p>
                            </div>
                        <?php else: ?>
                            <iframe src="https://view.officeapps.live.com/op/view.aspx?src=<?= urlencode($fullUrl) ?>" width="100%" height="100%" style="border: none;"></iframe>
                        <?php endif; ?>
                    <?php else: ?>
                        <div class="d-flex flex-column align-items-center justify-content-center h-100 text-white p-5 text-center">
                            <i class="bi bi-file-earmark-x fs-1 mb-3"></i>
                            <p>L'aperçu n'est pas disponible pour ce format de fichier.</p>
                        </div>
                    <?php endif; ?>
                <?php endif; ?>
            </div>
...
<?php else: ?>
    <!-- Cas où le besoin n'a ni fichier ni dossier -->
    <div class="alert alert-secondary border-0 py-2 small mb-0 d-inline-block">
        <i class="bi bi-info-circle me-1"></i> Aucun fichier n'est rattaché à ce besoin.
    </div>
<?php endif; ?>
