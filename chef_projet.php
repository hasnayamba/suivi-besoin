...
// ==================================================================
// FONCTIONS DE GESTION DES DOSSIERS PAR TITRE
// ==================================================================
...
/**
 * Génère un nom de dossier unique basé sur le titre.
 * Vérifie l'unicité dans la colonne dossier_upload de la table besoins.
 */
function genererNomDossierBesoin($pdo, $titre, $besoin_id = null) {
    $base = slugify($titre);
    if (empty($base)) $base = 'document';
    
    $nom_dossier = $base;
    $suffixe = 1;
    
    $sql = "SELECT id FROM besoins WHERE dossier_upload = ?";
    $params = [$nom_dossier];
    if ($besoin_id) {
        $sql .= " AND id != ?";
        $params[] = $besoin_id;
...
/**
 * Retourne le chemin absolu du dossier de stockage pour un besoin.
 * Crée le dossier s'il n'existe pas.
 */
function getDossierUploadPath($dossier_upload) {
    $baseDir = __DIR__ . '/uploads/';
    $dir = $baseDir . $dossier_upload . '/';
    if (!is_dir($dir)) {
        mkdir($dir, 0755, true);
    }
    return $dir;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_besoin'])) {
    ...
    $fichier_nom = null;
    $dossier_upload = null; // Nouveau champ
    ...
    if ($type_demande === 'Achat_Direct') {
        ...
        // --- Gestion de l'upload avec dossier personnalisé ---
        if (isset($_FILES['besoinFichier'])) {
            if ($_FILES['besoinFichier']['error'] !== UPLOAD_ERR_OK) {
                $uploadErrors = [
                ...
                $code = $_FILES['besoinFichier']['error'];
                $errorMsg = $uploadErrors[$code] ?? "Erreur inconnue (code $code)";
                $_SESSION['error'] = "Erreur d'upload : $errorMsg";
            } else {
                $ext = strtolower(pathinfo($_FILES['besoinFichier']['name'], PATHINFO_EXTENSION));
                if (in_array($ext, ['pdf', 'doc', 'docx', 'jpg', 'png', 'jpeg'])) {
                    // Générer un nom de dossier unique à partir du titre
                    $dossier_upload = genererNomDossierBesoin($pdo, $titre);
                    $uploadDir = getDossierUploadPath($dossier_upload);
                    
                    $nouvel_id = generateBesoinId(); // ID déjà généré plus tard, mais on peut le faire maintenant
                    $newFileName = $nouvel_id . '.' . $ext; // ou simplement 'document.' . $ext
                    if (move_uploaded_file($_FILES['besoinFichier']['tmp_name'], $uploadDir . $newFileName)) {
                        $fichier_nom = $newFileName;
                    } else {
                        $_SESSION['error'] = "Échec du déplacement du fichier vers le dossier 'uploads'.";
                    }
                } else {
                    $_SESSION['error'] = "Extension de fichier non autorisée. Utilisez PDF, DOC, DOCX, JPG, PNG ou JPEG.";
                }
            }
        } else {
            $_SESSION['error'] = "Aucun fichier reçu. Veuillez sélectionner un fichier.";
        }
    }

    // Si une erreur d'upload s'est produite, on arrête le traitement
    ...
    try {
        ...
        // L'ID est généré maintenant si ce n'est pas déjà fait
        if (empty($nouvel_id)) {
            $nouvel_id = generateBesoinId();
        }

        // Requête INSERT avec la colonne dossier_upload
        $sql = "INSERT INTO besoins (id, titre, description, montant, date_soumission, statut, fichier, dossier_upload, utilisateur_id, type_demande, projet_id, ligne_imputation, delai_souhaite, confirmation_pro, besoin_assistance) 
                VALUES (?, ?, ?, ?, CURDATE(), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([
            $nouvel_id, $titre, $description, $montant,
            $statut_initial,
            $fichier_nom,
            $dossier_upload, // peut être null pour Achat_Direct
            $utilisateur_id,
            $type_demande,
            $projet_id,
            $ligne_imputation,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_besoin'])) {
    ...
    $dossier_upload = null; // on le récupérera plus tard
    ...
    if ($type_demande === 'Achat_Direct') {
        ...
        // Gestion du fichier pour les corrections
        if (isset($_FILES['edit_fichier']) && $_FILES['edit_fichier']['error'] === UPLOAD_ERR_OK) {
            $ext = strtolower(pathinfo($_FILES['edit_fichier']['name'], PATHINFO_EXTENSION));
            if (in_array($ext, ['pdf', 'doc', 'docx', 'jpg', 'png', 'jpeg'])) {
                // Récupérer le dossier existant ou en créer un nouveau
                $stmt_dossier = $pdo->prepare("SELECT dossier_upload FROM besoins WHERE id = ?");
                $stmt_dossier->execute([$edit_id]);
                $dossier_upload = $stmt_dossier->fetchColumn();
                
                if (empty($dossier_upload)) {
                    // Générer un nouveau dossier basé sur le titre
                    $dossier_upload = genererNomDossierBesoin($pdo, $titre, $edit_id);
                    // On met à jour la colonne dossier_upload dans la base
                    $updateDossier = $pdo->prepare("UPDATE besoins SET dossier_upload = ? WHERE id = ?");
                    $updateDossier->execute([$dossier_upload, $edit_id]);
                }
                
                $uploadDir = getDossierUploadPath($dossier_upload);
                $newFileName = $edit_id . '_V2.' . $ext;
                if (move_uploaded_file($_FILES['edit_fichier']['tmp_name'], $uploadDir . $newFileName)) {
                    $fichier_sql = ", fichier = ?";
...
<head>
    ...
    <title>Espace Iniateur de besoin - Swisscontact</title>
    ...
    <style>
        ...
        .sidebar .p-4 { background-color: #ffffff; color: white; }
...
<div class="d-flex vh-100">
    <div class="collapse d-md-block" id="sidebarCollapse">
        <nav class="sidebar bg-white border-end">
            <div class="p-4 border-bottom"><img src="logo.png" alt="Swisscontact" style="height: 28px; width: auto; object-fit: contain;"></div>
            <div class="p-3">
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content shadow-lg border-0">
            ...
            <div class="modal-body p-4 bg-white">
                ...
                <!-- Section fichier avec deux boutons : visualiser et télécharger -->
                <div id="vFileSec" class="mt-4 d-none text-center bg-light p-3 rounded border">
                    <i class="bi bi-file-earmark-pdf-fill fs-2 text-danger d-block mb-2"></i>
                    <label class="small text-muted fw-bold d-block mb-2">Document rattaché (TDR, Devis...)</label>
                    <div class="d-flex justify-content-center gap-2">
                        <a id="vFileViewLink" href="#" class="btn btn-outline-primary fw-bold px-4" target="_blank">
                            <i class="bi bi-eye me-2"></i>Visualiser
                        </a>
                        <a id="vFileDownloadLink" href="#" class="btn btn-primary fw-bold px-4" download>
...
<script>
    ...
    document.getElementById('viewBesoinModal').addEventListener('show.bs.modal', function(event) {
        ...
        if (fichier && fichier !== 'null' && fichier !== '') {
            const fileUrl = 'uploads/' + fichier;
            document.getElementById('vFileViewLink').href = fileUrl;
            document.getElementById('vFileDownloadLink').href = fileUrl;
